...
class Parser(sly.Parser):
    ...
    @_('{ func_def }')
    def program(self, p):
        return [func_def for func_def, in p[0]]

    @_('KW_DEF IDENT "(" param_list ")" "{" { stmt } "}"')
    def func_def(self, p):
        body = [stmt for stmt, in p[6]]
        return (p.KW_DEF, p.IDENT, p.param_list, body)
    ...
    @_('"{" { stmt } "}"')
    def stmt(self, p):
        return [stmt for stmt, in p[1]]

    @_('prim_ty IDENT { "[" INT_CONST "]" }')
    def var_decl(self, p):
        return ('var', p[0], p[1], [c for _, c, _ in p[2]])
    ...
    def print_stmt(self, p):
        return (p.KW_PRINT, p.expr)

    @_('KW_READ lvalue')
    def read_stmt(self, p):
        return (p.KW_READ, p.lvalue)

    @_('KW_RETURN [ expr ]')
    def return_stmt(self, p):
        return (p.KW_RETURN, p[1][0])

    @_('KW_IF "(" expr ")" stmt [ KW_ELSE stmt ]')
    def if_stmt(self, p):
        else_stmt = p[5][1] if p[5] else None
        return (p.KW_IF, p.expr, p.stmt0, else_stmt)

    @_('KW_FOR "(" attrib_stmt ";" expr ";" attrib_stmt ")" stmt')
    def for_stmt(self, p):
        return (p.KW_FOR, p.attrib_stmt0, p.expr, p.attrib_stmt1, p.stmt)

    @_('IDENT "(" args_list ")"')
    def func_call(self, p):
        return ('call', p[0], p.args_list)
    ...
    @_(
        'INT_CONST', 'FLOAT_CONST', 'STRING_CONST', 'KW_NULL',
        'func_call', 'lvalue',
    )
    def expr(self, p):
        return p[0]
